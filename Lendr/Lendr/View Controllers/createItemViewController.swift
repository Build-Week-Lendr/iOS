...
class createItemViewController: UIViewController{
    ...
    var item: Item?
//    (name: "Thom", id: 99, itemDescription: "Fun", lendNotes: "rad", lentDate: Date(), context: CoreDataStack.shared.mainContext)
...
//        print("\(itemController)")
    ...
    @IBAction func saveButtonTapped(_ sender: Any) {
       ...
       let borrower = User(name: borrowerName, context: CoreDataStack.shared.mainContext)
        ...
        itemController.createItem(named: name, holder: borrower, itemDescription: itemDescription, lendNotes: notes, lendDate: Date(), context: CoreDataStack.shared.mainContext)
       navigationController?.popViewController(animated: true)
        ...
        guard let name = nameTextField.text,
          ...
          let notes = notesTextView.text ?? ""
            ...
            itemController.createItem(named: name, holder: nil, itemDescription: itemDescription, lendNotes: notes, lendDate: Date(), context: CoreDataStack.shared.mainContext)
         navigationController?.popViewController(animated: true)
        }
    }
